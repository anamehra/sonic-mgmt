...
create_sonic_topo:
	...
	bash -c "${PYTHON} update_topo.py -t ${TOPOLOGY} -p ${PLATFORM} --dut-username=${DUT_USERNAME} --goldencode=$(GOLDENCODE) --dut-password=${DUT_PASSWORD} $(PTF_IMAGE_ARG) ${DISABLE_ZTP_COMMAND}"
	bash -c " \
	 cd infra; \
