"""
2x2 VXLAN L3VNI Queue Occupancy / Watermark Measurement Test
...
Counterpart to scheduler/test_v6_queue_occ_wm_1node.py, but on the full
2-spine + 2-leaf L3VNI topology.

Topology (same as test_v6_ecn_vxlan_l3vni_2x2.py):
    SD1 -- Spine0   - D1
    SD2 -- Spine1   - D2
    SD3 -- Leaf0    - D3 (ingress leaf)
    SD4 -- Leaf1    - D4 (egress leaf)

Data path (single L3 routed flow via VRF / EVPN Type-5):
    T1D3P1 --> Leaf0 (Vrf01/Vlan2) --VXLAN--> Spine --> Leaf1 (Vrf01/Vlan3) --> T1D4P1

For every (frame_size, rate) combination in PACKET_SIZES x rates
...
Watermark egress ports (each node's downstream-facing port):
    leaf0   : D3D1P1 (+ D3D2P1 if present) -- to spine
...
# ---------------------------------------------------------------------------
# TGEN endpoint addressing -- reuse the same Vrf01 SVIs as the ECN L3VNI test
# (Vlan2 / 2002:db8:1::/64 on leaf0, Vlan3 / 2003:db8:1::/64 on leaf1).
# ---------------------------------------------------------------------------
T1D3P1_IP6 = "2002:db8:1::2"
T1D3P1_MAC = "00:0a:01:00:11:01"
D3T1_GW    = "2002:db8:1::1"

T1D4P1_IP6 = "2003:db8:1::2"
T1D4P1_MAC = "00:0a:01:00:12:01"
D4T1_GW    = "2003:db8:1::1"
...
def _verify_l3vni_asic_entries(nodes):
    """Verify L3VNI ASIC programming (VNI -> VRF mapping) on both leaves."""
    all_good = True
    for node_name in ('leaf0', 'leaf1'):
        ...
        has_vrf_entry = False
        for key in [line.strip() for line in output.split('\n')
            ...
            if 'VIRTUAL_ROUTER' in val:
                ...
                break
        if has_vrf_entry:
            st.log(f"{node_name}: L3VNI ASIC entries present")
        else:
...
def _resolve_tgen_gateway_nd(handles):
    """L3VNI requires gateway MAC resolution via ND before end-to-end ping."""
    for port, gw in (('T1D3P1', D3T1_GW), ('T1D4P1', D4T1_GW)):
        st.log(f"Pinging gateway {gw} from {port} to trigger ND...")
        ok = vxlan_obj.ping_gateway(handles, port, gw, handles[port]['int_handle'])
        st.log(f"{port} -> {gw}: {'OK' if ok else 'FAILED (non-fatal)'}")
...
def vxlan_queue_occ_module_setup():
    ...
    # Step 4: Generate and apply L3VNI VXLAN/BGP config
    st.banner("STEP 4: Applying L3VNI VXLAN/BGP configuration")
    ...
    st.banner("STEP 5: Waiting for BGP underlay + VXLAN tunnels")
    st.wait(20)
    ...
    _verify_l3vni_asic_entries(nodes)

    # Step 6: Discover ECN/lossless queue config on each monitored egress port
    ...
    # Use leaf0's discovery as the stream-side TC/DSCP (consistent with
    # ECN tests, which align lossless TC across the fabric).
    leaf0_cfg = data.ecn_cfg_per_node.get('leaf0')
    if not leaf0_cfg:
        st.report_fail('msg', "leaf0 ECN queue config discovery failed")
    data.tc = int(leaf0_cfg['tc'])
    data.dscp = int(leaf0_cfg['dscp'])
    st.log(f"Stream tagging: TC={data.tc} DSCP={data.dscp}")

    # Step 7: Reduce poll intervals on all monitored DUTs for snappy WM reads
    ...
    # Step 8: TGEN NGPF setup + gateway ND
    st.banner("STEP 8: Configuring TGEN NGPF and resolving gateways")
    int_dict = {
        'T1D3P1': {'host_ip': T1D3P1_IP6, 'gateway': D3T1_GW, 'mac': T1D3P1_MAC},
        'T1D4P1': {'host_ip': T1D4P1_IP6, 'gateway': D4T1_GW, 'mac': T1D4P1_MAC},
    }
    ...
    st.wait(10)
    _resolve_tgen_gateway_nd(handles)
    # Validate the L3VNI data path before declaring setup complete. A failure
    # here means traffic-driven readings would all be zero and the test would
    # falsely pass; one retry covers ND/EVPN-learn races right after start.
    ping_kwargs = dict(
        src_obj=tg,
    ...
    if not stream_api.verify_interface_ping(**ping_kwargs):
        ...
        if not stream_api.verify_interface_ping(**ping_kwargs):
            st.report_fail('msg',
                "L3VNI E2E ping T1D3P1 -> T1D4P1 failed; data path is broken")
...
def test_l3vni_vxlan_queue_occupancy_watermark():
    nodes = get_nodes()
    ...
    st.banner("SUMMARY: VXLAN L3VNI queue occ/watermark (per read)")
