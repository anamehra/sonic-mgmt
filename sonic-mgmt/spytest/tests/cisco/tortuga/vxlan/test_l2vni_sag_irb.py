...
def test_l2vni_sym_irb_sag_change_ip():
    ...
    # is raised before / during traffic_setup(). Same pattern as the IPv6
    # SAG IRB sibling (test_l2vni_ipv6_sym_irb_sag_change_ip).
    streams = None
    handles = None
    # The new SAG IP and the in-memory data.d3tp3_ip_addr / data.d4tp4_ip_addr
    ...
    # Wrap mutate+verify+traffic in try/finally so a verify_vtep_state /
...
def test_l2vni_sym_irb_sag_change_mac():
    ...
    # traffic_cleanup unconditionally without NameError when the exception
    # is raised before / during traffic_setup(). Same pattern as the IPv6
    # SAG IRB sibling (test_l2vni_ipv6_sym_irb_sag_change_mac).
    streams = None
    handles = None
    # The SAG MAC on both leafs must always be restored to SAG_MAC. Wrap
    # mutate+verify+traffic in try/finally so a verify_vtep_state /
